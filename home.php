...
<section class="view_posts">
   ...
   <div class="box-container">
      ...
      <?php
         $select_view_posts = $conn->prepare("SELECT * FROM `post` WHERE status = ? ORDER BY date DESC LIMIT 6");
         $select_view_posts = $conn->prepare("SELECT * FROM `post` WHERE approval = ? ORDER BY date DESC LIMIT 6");
         $select_view_posts->execute(['active']);
         $select_view_posts->execute(['1']);
