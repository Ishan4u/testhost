...
if(isset($_POST['submit'])){
   ...
   if(!empty($contact)){
      $update_contact = $conn->prepare("UPDATE `admin` SET contact = ? WHERE id = ?");
      $update_contact->execute([$contact, $admin_id]);
      $message[] = 'contact.no updated successfully!';
   }

   if(!empty($location)){
      $update_location = $conn->prepare("UPDATE `admin` SET location = ? WHERE id = ?");
      $update_location->execute([$location, $admin_id]);
   ...
   <form class="register" action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex">
         <div class="col">
            ...
            <input type="text" name="profession" placeholder="<?= $fetch_profile['profession']; ?>" maxlength="50" class="box">
            <p>your email </p>
            <input type="email" name="email" placeholder="<?= $fetch_profile['email']; ?>" maxlength="20"  class="box">
            <p>your contact no <span>*</span></p>
            <input type="text" name="contact" placeholder="<?= $fetch_profile['contact']; ?>" maxlength="50" class="box">
         ...
         <div class="col">
            ...
            <input type="text" name="location" placeholder="<?= $fetch_profile['location']; ?>" maxlength="50" class="box">
         </div>
      </div>
      <p>update pic :</p>
